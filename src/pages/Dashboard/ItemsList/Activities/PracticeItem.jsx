          ...
          <div id={"deleteActivity" + practice.id} className='popUpWindow' onClick={() => PratiqueUtil.accor("deleteActivity" + practice.id)}>
              <div className='popUpWindow-inner' onClick={(e) => e.stopPropagation()}>
                  <IonContent>
                      ...
                      <IonButton color="tertiary" data-testid="deleteCancel" onClick={() => PratiqueUtil.accor("deleteActivity")}>{translate.getText("WEIGHT_PREF_CANCEL")}</IonButton>
                      <IonButton color="primary" data-testid="deleteConfirm" onClick={() => props.onRemovePractice(practice)}>{translate.getText("CONFIRM_DELETE")}</IonButton>
                  </IonContent>
              </div>
          </div>
      </div>
